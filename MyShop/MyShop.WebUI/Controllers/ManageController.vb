...
Public Class ManageController
    ...
    Public Async Function Index(message As System.Nullable(Of ManageMessageId)) As Task(Of ActionResult)
        ViewData!StatusMessage = If(message = ManageMessageId.ChangePasswordSuccess, "암호가 변경되었습니다.", If(message = ManageMessageId.SetPasswordSuccess, "암호가 설정되었습니다.", If(message = ManageMessageId.SetTwoFactorSuccess, "2단계 인증 공급자가 설정되었습니다.", If(message = ManageMessageId.[Error], "오류가 발생했습니다.", If(message = ManageMessageId.AddPhoneSuccess, "전화 번호가 추가되었습니다.", If(message = ManageMessageId.RemovePhoneSuccess, "전화 번호가 제거되었습니다.", ""))))))
    ...
    Public Async Function ManageLogins(message As System.Nullable(Of ManageMessageId)) As Task(Of ActionResult)
        ViewData!StatusMessage = If(message = ManageMessageId.RemoveLoginSuccess, "외부 로그인이 제거되었습니다.", If(message = ManageMessageId.[Error], "오류가 발생했습니다.", ""))
        Dim userInfo = Await UserManager.FindByIdAsync(User.Identity.GetUserId())
